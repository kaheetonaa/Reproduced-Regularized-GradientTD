...
data = {
    "QRC":        np.array([np.load("MountainCar-Data/"+str(i)+"dataQRC.npy") for i in [0,15,29]]).reshape(42,100),
    "QC":         np.array([np.load("MountainCar-Data/"+str(i)+"dataQC.npy") for i in [0,15,29]]).reshape(42,100),
    "Q-Learning": np.array([np.load("MountainCar-Data/"+str(i)+"dataQLearning.npy") for i in [0,15,29]]).reshape(42,100),
}
print(data)
colors = {
    "QRC":        "#2563EB",
...
plt.tight_layout()
plt.savefig("mountain_car.png", dpi=150, bbox_inches="tight")
print("Saved.")
